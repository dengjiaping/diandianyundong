...
*/
    ...
    public static int mixColors(float t, int rgb1, int rgb2) {
        int a1 = Color.alpha(rgb1);//(rgb1 >> 24) & 0xff;
        int r1 = Color.red(rgb1);//(rgb1 >> 16) & 0xff;
        int g1 = Color.green(rgb1);//(rgb1 >> 8) & 0xff;
        int b1 = Color.blue(rgb1);//rgb1 & 0xff;
        int a2 = Color.alpha(rgb2);//(rgb2 >> 24) & 0xff;
        int r2 = Color.red(rgb2);//(rgb2 >> 16) & 0xff;
        int g2 = Color.green(rgb2);//(rgb2 >> 8) & 0xff;
        int b2 = Color.blue(rgb2);//rgb2 & 0xff;
//		a1 = lerp(t, a1, a2);
        r1 = lerp(t, r1, r2);
        g1 = lerp(t, g1, g2);
        b1 = lerp(t, b1, b2);
        return Color.argb(a1, r1, g1, b1);//(a1 << 24) | (r1 << 16) | (g1 << 8) | b1;
    }
    ...
    public static int bilinearInterpolate(float x, float y, int nw, int ne, int sw, int se) {
        float m0, m1;
        int a0 = Color.alpha(nw);//(nw >> 24) & 0xff;
        int r0 = Color.red(nw);//(nw >> 16) & 0xff;
        int g0 = Color.green(nw);//(nw >> 8) & 0xff;
        int b0 = Color.blue(nw);//nw & 0xff;
        int a1 = Color.alpha(ne);//(ne >> 24) & 0xff;
        int r1 = Color.red(ne);//(ne >> 16) & 0xff;
        int g1 = Color.green(ne);//(ne >> 8) & 0xff;
        int b1 = Color.blue(ne);//ne & 0xff;
        int a2 = Color.alpha(sw);//(sw >> 24) & 0xff;
        int r2 = Color.red(sw);//(sw >> 16) & 0xff;
        int g2 = Color.green(sw);//(sw >> 8) & 0xff;
        int b2 = Color.blue(sw);//sw & 0xff;
        int a3 = Color.alpha(se);//(se >> 24) & 0xff;
        int r3 = Color.red(se);//(se >> 16) & 0xff;
        int g3 = Color.green(se);//(se >> 8) & 0xff;
        int b3 = Color.blue(se);//se & 0xff;
        ...
        return Color.argb(a, r, g, b);//(a << 24) | (r << 16) | (g << 8) | b;
    }
    ...
    public static int brightnessNTSC(int rgb) {
        int r = Color.red(rgb);//(rgb >> 16) & 0xff;
        int g = Color.green(rgb);//(rgb >> 8) & 0xff;
        int b = Color.blue(rgb);//rgb & 0xff;
        return Color.argb(255, r, g, b);//(int)(r*0.299f + g*0.587f + b*0.114f);
    }
...
//			in[j] = ImageMath.clamp( in[j], 0, length-1 );
        ...
        rgb = source[srcIndex];
        a = Color.alpha(rgb);//(rgb >> 24) & 0xff;
        r = Color.red(rgb);//(rgb >> 16) & 0xff;
        g = Color.green(rgb);//(rgb >> 8) & 0xff;
        b = Color.blue(rgb);//rgb & 0xff;
        srcIndex += stride;
        rgb = source[srcIndex];
        nextA = Color.alpha(rgb);//(rgb >> 24) & 0xff;
        nextR = Color.red(rgb);//(rgb >> 16) & 0xff;
        nextG = Color.green(rgb);//(rgb >> 8) & 0xff;
        nextB = Color.blue(rgb);//rgb & 0xff;
        srcIndex += stride;
        i = 1;
        ...
        while (i <= length) {
            ...
            if (inSegment < outSegment) {
                ...
                dest[destIndex] = Color.argb((int) Math.min(aSum / sizfac, 255), (int) Math.min(rSum / sizfac, 255),
                        (int) Math.min(gSum / sizfac, 255), (int) Math.min(bSum / sizfac, 255));
...
//	return Color.argb(a, r, g, b);//(a << 24) | (r << 16) | (g << 8) | b;
    ...
    public static void premultiply(int[] p, int offset, int length) {
        ...
        for (int i = offset; i < length; i++) {
            int rgb = p[i];
            int a = Color.alpha(rgb);//(rgb >> 24) & 0xff;
            int r = Color.red(rgb);//(rgb >> 16) & 0xff;
            int g = Color.green(rgb);//(rgb >> 8) & 0xff;
            int b = Color.blue(rgb);//rgb & 0xff;
            ...
            g *= f;
            b *= f;
            p[i] = Color.argb(a, r, g, b);//(a << 24) | (r << 16) | (g << 8) | b;
        }
    }
