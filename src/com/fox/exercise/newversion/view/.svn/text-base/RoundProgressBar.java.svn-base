...
public class RoundProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float dimensionSize = getResources().getDimension(R.dimen.sleep_text);
        float dimensionSize1 = getResources().getDimension(R.dimen.sleep_text1);
...
//		paint.setTextSize(textSize);
        ...
        float textWidth4 = paint.measureText(getResources().getString(R.string.sleep_xiaolv) + "%");   //测量字体宽度，我们需要根据字体的宽度设置在圆环中间

        if (textIsDisplayable && style == STROKE) {
            paint.setColor(getResources().getColor(R.color.white));
            paint.setTextSize(dimensionSize);
            canvas.drawText(sleepTime, centre - textWidth2 / 2, centre - dimensionSize - dimensionSize, paint); //画出日期
            canvas.drawText(getResources().getString(R.string.sleep_xiaolv), centre - textWidth4 / 2, centre - dimensionSize / 2, paint); //画出日期
            paint.setColor(getResources().getColor(R.color.sleep_yellow));
            paint.setTextSize(dimensionSize1);
            canvas.drawText(percent + "%", centre - textWidth1 / 2, centre + dimensionSize1 * 3 / 4, paint); //画出进度百分比
            paint.setColor(getResources().getColor(R.color.white));
            paint.setTextSize(dimensionSize);
