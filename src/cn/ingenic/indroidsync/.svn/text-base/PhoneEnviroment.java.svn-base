...
import cn.ingenic.indroidsync.Enviroment.ResourceManager;
...
import com.fox.exercise.R.string;
...
public class PhoneEnviroment extends Enviroment {
		...
		mResMgr = new ResourceManager(context) {
			...
			Notification getRetryFailedNotification() {
				...
				noti.setLatestEventInfo(mContext,
						mContext.getString(R.string.timout_noti_title),
						mContext.getString(R.string.timout_noti_content), null);

				// Notification.Builder build = new
				// Notification.Builder(mContext);
				// build.setOngoing(true);
				// build.setContentTitle("IndroidSync Timeout");
				// build.setContentText("IndroidSync running without BT connectivity for more than two hours");
				// // build.setContentInfo("touch me to connect");
				// build.setTicker("IndroidSync disable auto connect");
			...
			Toast getRetryToast(int reason) {
				...
				case DefaultSyncManager.NO_LOCKED_ADDRESS:
					str = mContext
							.getString(R.string.retry_connect_failed_with_no_address);
					break;
				}			
	...
	public void processBondRequest(String address) {
		...
		String name = BluetoothAdapter.getDefaultAdapter()
				.getRemoteDevice(address).getName();
		String msg = mContext.getString(R.string.bond_request_message, name);
		edit.putString(DeviceListActivity.EXTRA_DEVICE_NAME, name);
		edit.commit();
