...
public class DiskLruCache {
    ...
    private static final String CACHE_FILENAME_PREFIX = "icon_";
     ...
     * @return
     */
    public static DiskLruCache openCache(final Context context, File cacheDir, long maxByteSize) {
//    	Log.d(TAG, "cacheDir.exists()?"+cacheDir.exists());
        if (!cacheDir.exists()) {
            cacheDir.mkdirs();
            Log.d(TAG, "cacheDir made:" + cacheDir.getPath());
        }
        if (Utils.getUsableSpace(cacheDir) < maxByteSize) {
            ///@SportsApp.getInstance().showToast(R.string.low_memory);

            Log.e(TAG, "no enough disk space left ");
            return null;
        }
//        Log.d(TAG, "cacheDir exist, path:"+cacheDir.getPath());
//        Log.d(TAG, "cacheDir.isDirectory():"+cacheDir.isDirectory());
//        Log.d(TAG, "cacheDir.canWrite():"+cacheDir.canWrite());

        if (cacheDir.isDirectory() && cacheDir.canWrite()
                ) {
            Log.d(TAG, "cacheDir create:" + cacheDir.getPath());
...
//    	Log.d(TAG, "Utils.isExternalStorageRemovable():"+Utils.isExternalStorageRemovable());
...
//		urlName += ".jpg";
     ...
     * @return
     */
    public String createFilePath(String key, boolean filterUrl) {
        if (filterUrl) {
            return createFilePath(mCacheDir, key);
        }
//        return createFilePath(mCacheDir, fileNameString);
        return mCacheDir.getAbsolutePath() + File.separator + urlToFileName(key);
    }
